...
This paper uses glmFit-based pipeline, whereas others recommend using glmQFit as it is more accurate for controlling type 1 error - see comments  https://support.bioconductor.org/p/76790/#:~:text=glmQLFit%20uses%20the%20trended%20NB,a%20GLM%20%2D%20and%20that's%20it

```{r setup, include=FALSE,eval=T}
# set the current session
session_packages = c("knitr","kableExtra","stats","apeglm","preprocessCore",
                     "affy","NOISeq","edgeR","data.table","ggplot2","ggcorrplot",
                     "caret","dplyr","e1071", "metap","stringr","ggrepel")
...
data_out_bucket = "gs://my-bucket/test/epigen-rrbs/mawg/data_03-27-23/"
dea_out_bucket = "gs://my-bucket/test/epigen-rrbs/mawg/dea_03-27-23/"
```
...
if(load_processed_data_from_rdata &&
  ...
  load("tissue2rrbs_processed_data2.RData")  
    ...
    for (tissue in tissues){
      ...
      keep[Chr=="chrM"] <- FALSE
      #remove sex chromosomes for the PCA per Yongchao's suggestion
      keep[Chr=="chrX"] <- FALSE
      keep[Chr=="chrY"] <- FALSE
...
# a helper function to run MDS on the top coefficient of variation features
load("motrpac-bic-norm-qc/bic_qc_reports/inprogress/tissue2rrbs_processed_data2.RData")
samples_to_remove=c()
...
for(tissue in names(tissue2Mmatrix)){
  ...
  df = data.frame(curr_pcax,
    ...
    id = pheno_data[rownames(curr_pcax),"viallabel"],
    stringsAsFactors = F
  )
  df$sex[df$sex=="1"] = "F"
  df$sex[df$sex=="2"] = "M"
  flag_samples=c(90266016903,90218016903,90578016903,90450015903,90585017004,90406015203,90251016604)
  # check data type of id column and cast it to character
if (class(df$id) != "character") {
  df$id <- as.character(df$id)
}

  df$id <- ifelse(df$id %in% flag_samples, df$id, NA)
  p = ggplot(df) + 
      geom_point(aes(x=PC1, y=PC2,col=randgroup, shape=sex)) +
      ggtitle(tissue) 
  p=p+geom_label_repel(aes(x=PC1,y=PC2,label = id),
                  box.padding   = 1, 
                  point.padding = 0.5,
...
for(currname in names(tissue2pca)){
  ...
  for(j in 1:ncol(curr_pcax)){
    ...
    for(i in seq_along(outlier_values)){
      outlier = paste("outlier", i, sep="")
      row_index = which(curr_pcax[,j] == outlier_values[i])
      row_name = rownames(curr_pcax)[row_index]
      pca_outliers_report = rbind(pca_outliers_report,
       c(currname,paste("PC",j,sep=""),
         row_name, format(outlier_values[i],digits=5))
      )
      if(!is.element(outlier,names(pca_outliers))){
        pca_outliers[[outlier]] = outlier_values[i]
      }
    }
  }
  ...
  if(length(pca_outliers)>0){
    ...
    df = data.frame(curr_pcax)
    # Create a new column to indicate if a row is an outlier
    df$outliers <- rownames(df) %in% pca_outliers_report[, 3]
    
    # Create a new column to indicate the color of the points
    df$color <- ifelse(df$outliers, "green", "black")
    
    # Plot PC1 vs. PC2 with outlier points in green
    plot(df$PC1, df$PC2, pch=19, col=df$color, lwd=2, cex=1,
       main=paste(currname, "flagged outliers"),
       xlab="PC1", ylab="PC2")
    
    # Add a legend to the plot
    legend("topleft", legend=c("Outliers", "Non-Outliers"), 
         col=c("green", "black"), pch=19, bty="n")
    
    # Plot PC3 vs. PC4 with outlier points in green
    plot(df$PC3, df$PC4, pch=19, col=df$color, lwd=2, cex=1,
       main=paste(currname, "flagged outliers"),
       xlab="PC3", ylab="PC4")
